...
from typing import Optional, Type, Dict, Union, TypeVar
Message = TypeVar('MsgType')
...
def create_bridge(factory: Union[str, "Bridge"], msg_type: Union[str, Type[Message]], topic_from: str,
    ...
    if not issubclass(msg_type, Message):
        raise TypeError(
            "msg_type should be Message instance or its string"
            "reprensentation")
    return factory(
...
class RosToMqttBridge(Bridge):
    ...
    def __init__(self, topic_from: str, topic_to: str, msg_type: Message, frequency: Optional[float] = None):
        self._topic_from = topic_from
        self._topic_to = self._extract_private_path(topic_to)
        self._last_published = rclpy.get_time()
        self._interval = 0 if frequency is None else 1.0 / frequency
        rclpy.create_subscription(msg_type, topic_from, self._callback_ros)

    def _callback_ros(self, msg: Message):
        rclpy.logdebug("ROS received from {}".format(self._topic_from))
        now = rclpy.get_time()
...
class MqttToRosBridge(Bridge):
    ...
    def __init__(self, topic_from: str, topic_to: str, msg_type: Type[Message],
        ...
        self._publisher = rclpy.create_publisher(
    ...
    def _callback_mqtt(self, client: mqtt.Client, userdata: Dict, mqtt_msg: mqtt.MQTTMessage):
        """ callback from MQTT """
        rclpy.logdebug("MQTT received from {}".format(mqtt_msg.topic))
        now = rclpy.get_time()
    ...
    def _create_ros_message(self, mqtt_msg: mqtt.MQTTMessage) -> Message:
